...
var PsiTurk = function (uniqueId, adServerLoc, mode) {
  ...
  var TaskData = Backbone.Model.extend({
    ...
    initialize: function () {
      ...
      this.listenTo(Backbone.Notifications, '_psiturk_lostfocus', function () {
        this.addEvent('focus', 'off');
      });
      this.listenTo(Backbone.Notifications, '_psiturk_gainedfocus', function () {
        this.addEvent('focus', 'on');
      });
      this.listenTo(Backbone.Notifications, '_psiturk_windowresize', function (newsize) {
        this.addEvent('window_resize', newsize);
      });
    },
  ...
  var Instructions = function (parent, pages, callback) {
    ...
    self.getIndicator = function () {
      return {
        currently_viewing: { indexOf: currentscreen, template: pages[currentscreen] },
        instruction_deck: { total_pages: instruction_pages.length, templates: instruction_pages },
      };
    };
  ...
  self.getPage = function (pagename) {
    if (!(pagename in self.pages)) {
      throw new Error(['Attemping to load page before preloading: ', pagename].join(''));
    }
  ...
  self.startTask = function () {
    ...
    if (self.taskdata.mode != 'debug') {
      // don't block people from reloading in debug mode
      // Provide opt-out
  ...
  self.completeHIT = function () {
    ...
    window.location =
      self.taskdata.adServerLoc + '?uniqueId=' + self.taskdata.id + '&mode=' + self.taskdata.mode;
  };
  ...
  var triggerResize = function () {
    Backbone.Notifications.trigger('_psiturk_windowresize', [
      window.innerWidth,
      window.innerHeight,
    ]);
  };
