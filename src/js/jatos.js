...
function submitFunc(maxAttempts = 3, timeout = 3000) {
  ...
  const submit = function (data, onSuccess, onFail) {
    if (attempt <= maxAttempts) {
      ...
      jatos.submitResultData(data, onSuccess, () =>
        setTimeout(function () {
          submit(data, onSuccess, onFail);
        }, timeout)
      );
    } else {
...
function onFinishedHandler(data, context) {
  ...
  if (abortedByUser) {
    ...
    onFail = () => {
      transferModal.open(
        'Failed to gracefully abort',
        'You can close this browser tab or window.',
        'error'
      );
    };
